namespace xHelix.Foundation.Theming.Extensions
{
    ...
    using xHelix.Foundation.Theming.Extensions.Controls;
    ...
    public static class RenderingExtensions
    {
        public static CarouselOptions GetCarouselOptions([NotNull] this Rendering rendering)
        {
            ...
            {
                ItemsShown = rendering.GetIntegerParameter(BusinessConstants.CarouselLayoutParameters.ItemsShown, 3),
                AutoPlay = rendering.GetIntegerParameter(BusinessConstants.CarouselLayoutParameters.Autoplay, 1) == 1,
                ShowNavigation = rendering.GetIntegerParameter(BusinessConstants.CarouselLayoutParameters.ShowNavigation) == 1
            };
        }

        public static string GetBackgroundClass([NotNull] this Rendering rendering)
        {
            var id = MainUtil.GetID(rendering.Parameters[BusinessConstants.BackgroundLayoutParameters.Background] ?? "", null);
        ...
        {
            var isFixed = MainUtil.GetBool(rendering.Parameters[BusinessConstants.IsFixedHeightLayoutParameters.FixedHeight] ?? "", false);
            return isFixed;
        }

        public static int GetHeight([NotNull] this Rendering rendering)
        {
            return MainUtil.GetInt(rendering.Parameters[BusinessConstants.IsFixedHeightLayoutParameters.Height] ?? "", 0);
        }
        ...
        {
            return MainUtil.GetBool(rendering.Parameters[BusinessConstants.HasContainerLayoutParameters.IsFluid], false);
        }
